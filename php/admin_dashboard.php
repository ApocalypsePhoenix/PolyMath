 ...
 * Image Processor
 */
function pm_process_img($base64, $existing = null) {
    if (empty($base64) || !str_contains($base64, ',')) return $existing;
    $data = explode(',', $base64);
    $imgData = base64_decode($data[1]);
    $name = uniqid('pm_', true) . '.png';
    ...
    try {
        if ($action === 'add_class') {
            ...
            if ($status == 1) {
                ...
                $cnt->execute([$id]);
                if ($cnt->fetchColumn() < 10) throw new Exception("You need 10 questions to publish!");
            }
            $pdo->prepare("UPDATE quizzes SET is_published = ? WHERE quiz_id = ?")->execute([$status, $id]);
            $message = $status ? "Published! 🚀" : "Saved as Draft.";
        } elseif ($action === 'add_question' || $action === 'edit_question') {
            $quiz_id = $_POST['quiz_id'];
            $text = trim($_POST['question_text']);
            $q_img = pm_process_img($_POST['cropped_question_image'] ?? '');
            $s_img = pm_process_img($_POST['cropped_solution_image'] ?? '');
            
            if ($action === 'add_question') {
                $st = $pdo->prepare("SELECT level_id FROM quizzes WHERE quiz_id=?");
                $st->execute([$quiz_id]); $lvl = $st->fetchColumn();
                ...
                $pdo->prepare($sql)->execute([$quiz_id, $lvl, $text, $_POST['option_a'], $_POST['option_b'], $_POST['option_c'], $_POST['option_d'], $_POST['correct_option'], $_POST['solution_text'], $q_img, $s_img, $_SESSION['user_id']]);
            } else {
                $pdo->prepare("UPDATE questions SET question_text=?, option_a=?, option_b=?, option_c=?, option_d=?, correct_option=?, solution_text=? WHERE question_id=?")
                    ->execute([$text, $_POST['option_a'], $_POST['option_b'], $_POST['option_c'], $_POST['option_d'], $_POST['correct_option'], $_POST['solution_text'], $_POST['question_id']]);
            }
            $message = "Saved!";
...
<body class="pb-12">
    ...
    <div id="cropModal">
        <div class="bg-white rounded-[2rem] p-8 w-full max-w-2xl">
            <h3 class="text-2xl font-black mb-4 text-[#46178f]">Crop Tool</h3>
    ...
    <nav class="kahoot-purple text-white shadow-xl p-5 sticky top-0 z-40">
        <div class="max-w-7xl mx-auto flex justify-between items-center"><h1 class="text-2xl font-black italic tracking-tighter uppercase">PolyMath <span class="font-light text-fuchsia-300">Admin</span></h1><a href="logout.php" class="bg-red-500 hover:bg-red-600 px-6 py-2.5 rounded-xl text-sm font-black transition-all">LOGOUT</a></div>
    </nav>
            ...
            <div class="bg-white rounded-[3rem] p-12 shadow-2xl border-4 border-indigo-50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
                    ...
                    <div class="bg-indigo-50 p-6 rounded-3xl border-2 border-white shadow-sm text-center">
                        ...
                        <p class="text-[10px] font-bold text-indigo-300 mt-1 uppercase">Students</p>
                    </div>
                    <?php endforeach; ?>
                </div>

                <div class="space-y-8">
                    <h3 class="text-xl font-black flex items-center gap-3">📊 Intelligence: Question Success Graph</h3>
                    <?php foreach($q_stats as $st): 
                        $pct = round(($st['correct'] / $st['attempts']) * 100);
                        $col = ($pct > 75) ? 'bg-green-500' : (($pct > 40) ? 'bg-amber-500' : 'bg-red-500');
                    ?>
                    <div class="bg-gray-50 p-6 rounded-[2rem] border flex flex-col md:flex-row gap-8 items-center">
                        <div class="md:w-1/3">
                        ...
                        <div class="flex-grow w-full">
                            <div class="flex justify-between items-end mb-2">
                                ...
                                <span class="text-[10px] font-black text-gray-400 uppercase"><?php echo $st['unique_students']; ?> Students Attempted</span>
                            </div>
        ...
        <section id="sect-quizzes" class="tab-content space-y-12">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div class="bg-white rounded-[2rem] p-8 shadow-xl border-b-8 border-indigo-600 h-fit">
                    <h2 class="text-2xl font-black mb-6 text-indigo-700" id="quiz-form-title">Settings</h2>
                    <form action="admin_dashboard.php" method="POST" class="space-y-5">
                        ...
                        <div class="p-4 bg-indigo-50 rounded-2xl border-2">
                            <label class="flex items-center gap-3"><input type="checkbox" name="is_timed" id="quiz-timed" class="w-5 h-5 accent-indigo-600" onchange="document.getElementById('limit-box').classList.toggle('hidden', !this.checked)"><span class="text-sm font-black">Enable Timer</span></label>
                        ...
                        <button type="submit" class="w-full bg-[#1368ce] text-white py-4 rounded-2xl font-black shadow-lg">SAVE QUIZ</button>
                    </form>
                </div>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody>
                            ...
                            <tr class="hover:bg-gray-50 border-b">
                                <td class="px-8 py-6 font-black"><?php echo htmlspecialchars($q['quiz_name']); ?></td>
                                <td class="px-8 py-6 text-center font-bold text-gray-400"><?php echo $q['q_count']; ?>/10</td>
                                <td class="px-8 py-6"><?php echo ($q['is_published']) ? '<span class="bg-green-100 text-green-700 px-3 py-1 rounded-full text-[10px] font-black uppercase">LIVE</span>' : '<span class="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-[10px] font-black uppercase">DRAFT</span>'; ?></td>
                                <td class="px-8 py-6 text-right space-x-3">
                                    <button onclick='editQuiz(<?php echo json_encode($q); ?>)' class="text-indigo-600 font-black text-xs uppercase">Edit</button>
                                    <form action="admin_dashboard.php" method="POST" class="inline" onsubmit="return confirm('Delete?')"><input type="hidden" name="action" value="delete_quiz"><input type="hidden" name="quiz_id" value="<?php echo $q['quiz_id']; ?>"><button type="submit" class="text-red-500 font-black text-xs uppercase">Del</button></form>
                                    <button onclick="manageQuestions(<?php echo $q['quiz_id']; ?>, '<?php echo addslashes($q['quiz_name']); ?>', <?php echo $q['q_count']; ?>, <?php echo $q['is_published']; ?>)" class="kahoot-purple text-white px-5 py-2.5 rounded-xl text-xs font-black shadow-md">BUILD</button>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <!-- Builder Workspace -->
            <div id="builder" class="hidden bg-white rounded-[3rem] p-10 shadow-2xl border-4 border-[#46178f]">
                <div class="flex justify-between items-center mb-10 pb-6 border-b-2">
                    <h2 class="text-3xl font-black">Mission: <span id="managed-name" class="text-[#46178f]"></span></h2>
                    <div class="flex gap-4">
                        <form action="admin_dashboard.php" method="POST">
                            ...
                            <button type="submit" id="pub-btn" class="hidden px-8 py-4 rounded-2xl font-black text-white shadow-xl uppercase"></button>
                        </form>
                        <button onclick="document.getElementById('builder').classList.add('hidden')" class="kahoot-yellow text-white px-8 py-4 rounded-2xl font-black shadow-xl uppercase tracking-widest">SAVE AS DRAFT</button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-3 gap-12">
                    <form action="admin_dashboard.php" id="q-form" method="POST" class="space-y-6 bg-gray-50 p-8 rounded-[2rem] border-2">
                        ...
                        <textarea name="question_text" id="q-text" placeholder="Question Text" required class="w-full px-5 py-4 rounded-2xl border-2 font-bold h-32 focus:border-indigo-500 outline-none"></textarea>
                        ...
                        <select name="correct_option" id="q-correct" class="w-full px-5 py-3 rounded-2xl border-2 font-black">
                        ...
                        <textarea name="solution_text" id="q-sol" placeholder="Solution logic..." class="w-full px-5 py-4 rounded-2xl border-2 font-bold h-24"></textarea>
                        <button type="submit" class="w-full kahoot-purple text-white py-4 rounded-2xl font-black shadow-xl">UPLOAD CHALLENGE</button>
                    </form>
        ...
        <section id="sect-classes" class="tab-content hidden space-y-10">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div class="bg-white rounded-[2.5rem] p-8 shadow-xl border-b-8 border-green-600 h-fit">
                    <h2 class="text-2xl font-black mb-6 text-green-700">Add Class</h2>
                    <form action="admin_dashboard.php" method="POST" class="space-y-4"><input type="hidden" name="action" value="add_class"><input type="text" name="new_class_name" required placeholder="Group Name" class="w-full px-5 py-4 rounded-2xl border-2 font-bold"><button type="submit" class="w-full kahoot-green text-white py-4 rounded-2xl font-black">CREATE GROUP</button></form>
                </div>
                <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <?php foreach($classes as $c): ?>
                    <div class="bg-white rounded-[2rem] p-8 shadow-lg border flex items-center justify-between"><div><h3 class="text-2xl font-black text-gray-800"><?php echo htmlspecialchars($c['class_name']); ?></h3><p class="text-xs font-black text-indigo-500 uppercase"><?php echo $c['student_count']; ?> Students</p></div><form action="admin_dashboard.php" method="POST" onsubmit="return confirm('Delete?')"><input type="hidden" name="action" value="delete_class"><input type="hidden" name="class_id" value="<?php echo $c['class_id']; ?>"><button type="submit" class="w-10 h-10 bg-red-50 text-red-500 rounded-xl">🗑️</button></form></div>
                    <?php endforeach; ?>
                </div>
            </div>
        </section>
    ...
    <script>
        ...
        async function manageQuestions(quizId, quizName, qCount, isPub) {
            ...
            if(qCount >= 10) {
                ...
                if(isPub) { btn.innerText = "UNPUBLISH"; btn.className = "kahoot-red px-8 py-4 rounded-2xl font-black text-white"; val.value = 0; }
                else { btn.innerText = "PUBLISH LIVE"; btn.className = "kahoot-green px-8 py-4 rounded-2xl font-black text-white"; val.value = 1; }
            } else { btn.classList.add('hidden'); }

            const list = document.getElementById('questions-list');
            list.innerHTML = "<p class='p-10 text-center opacity-30 font-black uppercase'>Gathering Challenges...</p>";
            try {
                const res = await fetch(`get_questions.php?quiz_id=${quizId}`);
                const data = await res.json();
                list.innerHTML = "";
                data.forEach((q, i) => {
                    list.innerHTML += `<div class="bg-white p-6 rounded-2xl border-2 flex items-center gap-6 shadow-sm"><div class="w-10 h-10 bg-indigo-50 text-indigo-700 flex items-center justify-center font-black rounded-lg">${i+1}</div><div class="flex-grow font-bold text-gray-700 truncate">${q.question_text}</div><form action="admin_dashboard.php" method="POST" class="inline"><input type="hidden" name="action" value="delete_question"><input type="hidden" name="question_id" value="${q.question_id}"><button type="submit" class="text-red-500 font-black text-xs uppercase" onclick="return confirm('Del?')">Delete</button></form></div>`;
                });
